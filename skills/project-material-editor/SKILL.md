...
- `scripts/docx_inventory.py`: inspect `.docx` text, heading candidates, captions, media files, and term occurrences. Run `--help` for examples, stale-term search usage, defaults, and exit codes.
- `scripts/md_to_docx.py`: convert Markdown to DOCX from the project root. By default it uses a style-driven `python-docx` renderer for common report blocks, including headings, paragraphs, simple lists, local images, figure captions, table captions, and pipe tables; existing DOCX output is not overwritten unless `--overwrite` is passed. Use `--style-config` or the documented font, margin, and image-size flags for built-in renderer style tuning. Use `--pandoc` or `--reference-doc` when Pandoc coverage or a reference DOCX template is required; requested Pandoc conversion fails if Pandoc is unavailable instead of silently ignoring the template. Run `--help` before use for examples, dependencies, path rules, defaults, and exit codes.
- `scripts/docx_to_md.py`: convert DOCX to clean Markdown for review or recovery; it extracts common headings, paragraphs, lists, tables, hyperlinks, basic inline emphasis, and media references directly from the DOCX package. By default it writes `<stem>.md` when free or `<stem>.from-docx.md` instead of overwriting an existing `<stem>.md`; use `--overwrite` only when replacing the Markdown output is intended. Run `--help` for examples, media extraction defaults, dependencies, and exit codes.
