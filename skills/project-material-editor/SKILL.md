...
1. Establish the scope and authorities.
   ...
   - When Markdown or another text source generates DOCX/PPTX/PDF outputs, keep the export command, required tool, template or reference file, and output path discoverable in project-facing documentation or a generation script. Keep those operational notes out of final-facing prose unless the target document is itself a procedure, delivery guide, or maintenance note.
